...
def save_checkpoint(args, state, filename, recalls_rerank):
...
def compute_pca(args, model, pca_dataset_folder, full_features_dim):
    ...
    with torch.no_grad():
        for i, images in enumerate(tqdm(dl, desc="Extracting features")):
    ...
    os.makedirs(os.path.dirname(args.pca_path), exist_ok=True)
    with open(args.pca_path, "wb") as f:
        pickle.dump(pca, f)
    print(f"[Info] PCA model saved to {args.pca_path}")
